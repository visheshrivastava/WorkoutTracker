...
function checkAuth(req, res, next) {
    ...
    jwt.verify(authToken, process.env.JWT_ADMIN_SECRET_KEY, (err, decoded) => {
        if (err) {
            // Auth token has expired, check the refresh token
            jwt.verify(refreshToken, process.env.REFRESH_TOKEN_JWT_REFRESH_SECRET_KEY, (refreshErr, refreshDecoded) => {
                if (refreshErr) {
                    ...
                    const newAuthToken = jwt.sign({ userId: refreshDecoded.userId }, process.env.JWT_ADMIN_SECRET_KEY, { expiresIn: '10m' });
